...
import com.reggie.core.modular.common.constant.SymbolConstants;
...
//    }
    ...
    public double hdecr(String key, String item, long by) {
        try (Jedis jedis = getJedis()) {
            return hincr(key, item, MyNumberUtil.negative(by));
        }
    }
    ...
    public Set<String> getKeys(String pattern) {
        return redisTemplate.execute((RedisCallback<Set<String>>) connection -> {
            ...
            Cursor<byte[]> cursor = connection.scan(new ScanOptions.ScanOptionsBuilder()
                    .match(SymbolConstants.ASTERISK + pattern + SymbolConstants.ASTERISK)
                    .count(1000)
                    .build());
